...
from seminarapp3.models import Author3, Post3
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for i in range(1, count + 1):
            author = Author3(name=f'Author_{i}', email=f'mail{i}@mail.ru')
            author.save()
            for j in range(1, count + 1):
                post = Post3(title=f'Title-{j}', content=" ".join(choices(text, k=64)), author=author)
                post.save()
